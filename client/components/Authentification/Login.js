...
function Login() {
  ...
  const post = () => {
    axios
      .post(`http://192.168.11.203:3000/auth/Login`, { formData })
      .then((response) => {
        let errors = {};
    ...
    Google.logInAsync(config)
      .then((result) => {
        ...
        if (type == "success") {
          ...
          axios
            .post(`http://192.168.11.203:3000/auth/GoogleLogin`, {
              email,
            })
            .then((response) => {
              const data = response.data;
      ...
      <Center flex={1} px="3">
        <Modal isOpen={isOpen} onClose={onClose}>
          <Modal.Content>
            ...
            <Modal.Body>you will be redirected to the signup page</Modal.Body>
          </Modal.Content>
        </Modal>
        <Box safeArea p="2" py="8" w="120%" maxW="300">
          ...
          <VStack space={3} mt="5">
            ...
            <Divider my={2} />
            <Button mt="4" colorScheme="teal" onPress={handleGoogleSignin}>
              Sign in with Google
            </Button>
